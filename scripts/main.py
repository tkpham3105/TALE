...
def main():
    ...
    for files in sorted(os.listdir(opt.root)):
        torch.cuda.empty_cache()
        prompt = " ".join([w for w in files.split(" ")[1:] if len(w)>0])
        subdir = os.path.join(opt.root, files)
        ...
        for file in os.listdir(subdir):
            ...
            if file_path.endswith('.jpg') or file_path.endswith('.jpeg') or file_path.endswith('.png'):
                ...
                elif file.startswith('fore'):
                    opt.ref_img = file_path
                elif file.startswith('loc'):
                    opt.mask = file_path
        ...
        img = cv2.resize(np.array(img).astype(np.uint8), (512, 512), interpolation=cv2.INTER_NEAREST_EXACT)
        ...
        sampler.image_encoder.cal_ref(opt.init_img)
        ...
        ref_vis_box = [top_rr, bottom_rr, left_rr, right_rr, segmentation_map_save]

        bbox = [
            center_row_rm - step_height1,
            center_row_rm + step_height2, 
            center_col_rm - step_width1,
            center_col_rm + step_width2
        ]
        ...
        with precision_scope("cuda"):
            for prompts in data:
                ...
                samples = adaptive_normalization(samples.clone(), samples_orig, [param[0], param[1], param[2], param[3]], 1, alpha=0.5,mask=segmentation_map[:, :, top_rr:bottom_rr, left_rr:right_rr].bool())
                ...
                for x_sample in x_samples:
                    x_sample = 255. * rearrange(x_sample.detach().cpu().numpy(), 'c h w -> h w c')
